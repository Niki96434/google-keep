# Pig Keep
MVP веб-приложение с виртуальным питомцем, основной продукт которого - приложение заметок. За активность пользователь получает награды в виде XP и HP, повышает уровень питомца и вместе с тем помогает ему повышать шкалу удовольствия.

## Сценарий
1.Регистрация: пользователь регистрируется в приложении, используя почту/логин и пароль.В этот момент за ним закрепляется питомец с базовыми показателями.\
2.Базовый прогресс: активность пользователя за день подсчитывается и пользователь получает определённое количество XP, отвечающее за уровень удовольствия питомца и +3HP для поддержания жизни питомца.\
3.Органическая активность (скрытая механика): при использовании основного функционала (создание, редактирование, архивация заметок) у хрюшки растет скрытый параметр - индекс удовольствия.\
4.Если пользователь активно использует приложение, то у него есть возможность попасть в лидерборд, глобальный топ "самых довольных питомцев". Пользователи видят свою позицию в рейтинге, суммарный XP и аватар своего питомца.\
5.Если пользователь совсем не использует приложение заметок(основной продукт),то питомец теряет свои HP, вплоть до того, что питомец может умереть.

## Описание решения
— Проблема: какую проблему решает проект и каким образом;
— Идея решения: в чем заключается ваш подход и почему он должен сработать;
— MVP: что команда реализовала во время хакатона и какие ключевые функции уже работают;
— Ценность решения: в чем преимущество подхода и как решение может применяться в Авито.

## Перечень используемых технологий (с их обоснованием)

- React 19, Typescript, Vite, Eslint, Prettier, Vitest, RTL, RTQuery, Zod
- Express.js, Supertest, Drizzle ORM
- PostgreSQL 18

## Структура проекта

- apps/frontend/src - React-приложение
- apps/backend/core - Инициализация базы данных, глобальный обработчик ошибок, мидлвары
- apps/backend/features - feature-based модульная архитектура
- apps/backend/migrations - схемы PostgreSQL
- apps/packages/shared - общие типы и схемы контракта
- apps/docs - таблица с описанием эндпоинтов(REST), описание решения, диаграммы компонентов и развёртывания, дизайн мобильного интерфейса, на планшете и десктоп

## Инструкция по запуску(развёрнутый проект по ссылке и локальный)

## Особенности реализации

## Доп функционал(интеграция ИИ, auth, docker, виртуальный питомец тамагочи)

## Декомпозиция(архитектура и API)

## Использование ИИ

## История коммитов

## Дизайн
### Главный экран десктопного и мобильного экранов


## Скриншоты
